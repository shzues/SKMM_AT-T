...
@synthesize displayRightInfoButton;
...
- (void)viewDidLoad {
    ...
    //UINavigation add right item.
    if (displayRightInfoButton){
...
- (void)showInfoView:(id)sender{
    ...
    /*
    InfoDetailViewController *rvController = [[InfoDetailViewController alloc] init];
    
    rvController.cdDataResultsDetailArray = cdDataResultsArray;
    rvController.cdPhotoIndex = _currentPageIndex;
    
    //InfoDetailViewController *rvController = [self.storyboard instantiateViewControllerWithIdentifier:@"InfoDetailView"];
...
- (void)actuallyEmailPhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        ...
        [emailer setSubject:NSLocalizedString(@"Malaysia Wedding Gallery", nil)];
        [emailer addAttachmentData:UIImagePNGRepresentation([photo underlyingImage]) mimeType:@"png" fileName:@"MalaysiaWeddingGallery-Photo.png"];
        
        //Picture Profile.
        NSString *_picProfile = [NSString stringWithFormat:@"Malaysia Wedding Gallery: \n %@\n\n%@", [[cdDataResultsArray objectAtIndex:_currentPageIndex] objectForKey: @"pic_desc_title"], [[cdDataResultsArray objectAtIndex:_currentPageIndex] objectForKey: @"pic_desc"]];
        
        
        [emailer setMessageBody:_picProfile isHTML:NO]; //you can just nil it
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
